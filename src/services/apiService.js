...
API.interceptors.response.use(
    (response) => {
        ...
        if (response.config.url.includes('/courses/') && response.config.method === 'put') {
    ...
    async (error) => {
        if (error.response) {
            ...
            alert('Unable to connect to the server. Please check if the backend server is running at http://localhost:5186');
        } else {
