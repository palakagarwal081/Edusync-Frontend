...
const API = axios.create({
  baseURL:
    "https://edusyncbackendapp-epefcafwarh0e7g6.centralindia-01.azurewebsites.net", // Or use import.meta.env.VITE_API_BASE_URL for env flexibility
  headers: {
    "Content-Type": "application/json",
    Accept: "application/json",
  },
...
// Request interceptor for token injection & logging
API.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem("token");
    ...
    if (
      ...
      if (config.url.includes("/courses/") && config.method === "put") {
        ...
        console.warn("COURSE UPDATE REQUEST", {
          courseId: config.url.split("/courses/")[1],
          courseContent,
        });
      }
...
API.interceptors.response.use(
  (response) => {
    if (process.env.NODE_ENV === "development") {
      ...
      if (
        ...
        const courseContent = response.data?.courseContent || null;
        console.warn("COURSE UPDATE RESPONSE", {
          courseId: response.config.url.split("/courses/")[1],
          courseContent,
        });
      }
  ...
  (error) => {
    if (error.response) {
      ...
      alert("Backend server unreachable. Please try again later.");
    } else {
      console.error("Request setup error:", error.message);
    }
...
const apiService = {
  ...
  put: async (url, data) => {
    try {
      if (url.includes("/courses/")) {
        data.courseContent = data.courseContent?.trim() || null;

        if (process.env.NODE_ENV === "development") {
          console.warn("COURSE PUT REQUEST", {
            url,
            processedData: data,
          });
        }
